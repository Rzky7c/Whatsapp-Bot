...
exports.cmd = {
    ...
    detail: {
        desc: 'Information Server from Minecraft Java.',
        use: 'ip'
    },
    setting: {
        error_react: true
    ...
    async start({ msg, text }) {
        ...
        try {
            const res = await axios.get(`${website.web}/api/info/java?query=${encodeURIComponent(text)}&apikey=${website.apikey}`);
            ...
            let replyMessage = `🔍 *Hasil Information Minecraft Java*:\n\n`;
            if (typeof result === 'object' && result !== null) {
                const ip = result.ip || "❓ Tidak diketahui";
                const port = result.port || "🔒 #";
                const gamemode = result.software || "🚫 0";
