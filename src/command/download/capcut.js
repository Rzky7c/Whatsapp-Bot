...
exports.cmd = {
    ...
    category: ['download'],
    detail: {
        desc: 'Download Video from Capcut Video.',
        use: 'url'
    },
    setting: {
        error_react: true
    ...
    async start({ msg, text }) {
        if (!text) {
            return msg.reply(`✘ Masukkan URL video Capcut untuk mengunduh video.`);
        }
        if (!text.match(/capcut\.com/g)) {
            return msg.reply(`✘ Invalid url, masukkan url Capcut dengan benar`);
        }
        await msg.reply(`🕓 Tunggu sebentar file sedang dikirim.`);
        
        const start = Date.now();
        const apiUrl = `${website.web}/api/download/capcut?url=${text}&apikey=${website.apikey}`;
