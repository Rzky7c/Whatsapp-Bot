...
exports.cmd = {
    ...
    category: ['download'],
    detail: {
        desc: 'Download Video from Like Video.',
        use: 'url'
    },
    setting: {
        error_react: true
    ...
    async start({ msg, text }) {
        if (!text) {
            return msg.reply(`✘ Masukkan URL video Like untuk mengunduh video.`);
        }

        await msg.reply(`🕓 Tunggu sebentar file sedang dikirim.`);
        const start = Date.now();
        const apiUrl = `${website.web}/api/download/like?url=${text}&apikey=${website.apikey}`;
